    ...
    public class ExampleNetworkedEntityView : ColyseusNetworkedEntityView
    {
        ...
        IEnumerator Co_InitiWithServer()
        {
            ...
            if (autoInitEntity && HasInit == false && !string.IsNullOrEmpty(prefabName))
            {
                ...
                ExampleManager.CreateNetworkedEntity(prefabName, new Dictionary<string, object> { ["userName"] = UserName }, this);
            }
        ...
        protected void SetUserNameAttribute()
        {
            SetAttributes(new Dictionary<string, string> { ["userName"] = ExampleManager.Instance.UserName });
        }
        ...
        virtual protected void UpdateViewFromState()
        {
            ...
            //Debug.Log("Received update from remote player!");
        ...
        protected virtual void UpdateStateFromView()
        {
            ...
            state.xVel = localPositionDelta.x / Time.deltaTime;
            state.yVel = localPositionDelta.y / Time.deltaTime;
        ...
        protected virtual void ProcessViewSync()
        {
            ...
            //Debug.Log($"Checking if we should Sync views - State Timestamp: {proxyStates[0].timestamp}, Server Time: {ExampleManager.Instance.GetServerTime}, Server Time - Interpolation Back Time: {interpolationTime}, Round Trip Time: {ExampleManager.Instance.GetRoundtripTime}");

            // Use interpolation if the target playback time is present in the buffer
            if (proxyStates[0].timestamp > interpolationTime)
            {
                //Debug.Log("UPDATE REMOTE PLAYER!");
                ...
                if (syncLocalPosition) myTransform.localPosition = Vector3.Slerp(myTransform.localPosition, proxyStates[0].pos, Time.deltaTime * (positionLerpSpeed + deltaFactor));
