...
public partial class Damageable : MonoBehaviour
{
    ...
    public void ApplyDamage(DamageMessage data)
    {
        ...
        if (data.isRFC)
        {
            //We only send RFC if we've shot a playercontroller
            if (TryGetComponent(out PlayerSpaceshipController controller))
            {
                controller.ReceiveDamage(data.damager, data.amount);
            }
            return;
        }
